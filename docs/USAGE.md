...
- **Usage Summary** *(fork addition)* - One composite widget that renders the whole usage line at once, e.g. `5h:6% ends 19:30  context:96,124  max  $1.46  3d:16%`: 5-hour window used with its reset clock (local 24h `HH:MM`), current context length (comma-grouped), thinking-effort level, session cost, and — last — days remaining in the weekly window with weekly usage %. Segments with no data are skipped. See the [Fork personalizations](../README.md#-fork-personalizations) section.
