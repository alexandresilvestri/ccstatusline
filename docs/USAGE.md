...
- **Usage Summary** *(fork addition)* - One composite widget that renders the whole usage line at once, e.g. `5h:6% ends 19:30  3d:16%  context:96,124  $1.46`: 5-hour window used, its reset clock (local 24h `HH:MM`), days remaining in the weekly window with weekly usage %, current context length (comma-grouped), and session cost. Segments with no data are skipped. See the [Fork personalizations](../README.md#-fork-personalizations) section.
